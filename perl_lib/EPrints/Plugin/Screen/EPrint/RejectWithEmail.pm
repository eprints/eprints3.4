...
sub render
{
	...
	do {
		...
		my $edit_link_a = $self->{session}->make_element( "a", href=>"#", role=>"button", onclick => "EPJS_blur(event); EPJS_toggle('ep_mail_reason_fixed',true,'block');EPJS_toggle('ep_mail_reason_edit',false,'block');\$('ep_mail_reason_edit').focus(); \$('ep_mail_reason_edit').select(); return false", );
